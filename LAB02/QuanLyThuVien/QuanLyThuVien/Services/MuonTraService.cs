...
namespace QuanLyThuVien.Services
{
    public class MuonTraService
    {
        public KetQuaXuLy KiemTraDieuKienMuon(string maDocGia, int soSachMuonMoi)
        {
            ...
            if (soSachMuonMoi > 3) return KetQuaXuLy.Loi("Một lần lập phiếu chỉ được chọn tối đa 3 đầu sách khác nhau (BR04).");
            ...
            if (the.Rows.Count == 0) return KetQuaXuLy.Loi("Độc giả chưa có thẻ thư viện đang hoạt động (BR01).");

            DateTime han = Convert.ToDateTime(the.Rows[0]["HanSuDung"]);
            bool lePhi = Convert.ToBoolean(the.Rows[0]["DaDongLePhi"]);

            if (han.Date < DateTime.Today) return KetQuaXuLy.Loi("Thẻ thư viện đã hết hạn (BR03).");
            if (!lePhi) return KetQuaXuLy.Loi("Độc giả chưa đóng lệ phí năm nên thẻ chưa có giá trị (BR03).");
            ...
            if (quaHan > 0) return KetQuaXuLy.Loi("Độc giả còn sách quá hạn chưa trả nên không được mượn thêm (BR05).");
            ...
            if (dangMuon + soSachMuonMoi > 3)
                return KetQuaXuLy.Loi("Tổng số sách đang mượn (" + dangMuon + ") và sắp mượn (" + soSachMuonMoi + ") không được vượt quá 3 cuốn (BR04).");
        ...
        {
            if (maDauSach == null || maDauSach.Count == 0) return KetQuaXuLy.Loi("Danh sách sách mượn không hợp lệ.");
            ...
            if (ngayHenTra.Date < ngayMuon.Date) return KetQuaXuLy.Loi("Ngày hẹn trả không được trước ngày mượn (BR08).");
            ...
            using (var tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    {
                        using (var check = new SqlCommand("SELECT SoLuongHienCo FROM DauSach WITH (UPDLOCK, HOLDLOCK) WHERE MaDauSach=@Ma", cn, tx))
                        {
                            ...
                            if (Convert.ToInt32(o) <= 0) { tx.Rollback(); return KetQuaXuLy.Loi("Đầu sách " + maSach + " đã hết trong kho (BR07)."); }
                        }
                    }

                    string maPM = "PM" + DateTime.Now.ToString("yyyyMMddHHmmssfff");
                    using (var insPm = new SqlCommand(@"INSERT INTO PhieuMuon (MaPhieuMuon, MaDocGia, MaNhanVien, NgayMuon, NgayHenTra)
                    ...
                    foreach (string maSach in unique)
                    {
                        ...
                        using (var insCt = new SqlCommand(@"INSERT INTO ChiTietPhieuMuon (MaChiTiet, MaPhieuMuon, MaDauSach)
                        ...
                        using (var update = new SqlCommand(@"UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo-1 WHERE MaDauSach=@Ma AND SoLuongHienCo>0", cn, tx))
                        {
        ...
        public KetQuaXuLy TraSach(string maChiTiet, string maNhanVien, DateTime ngayTra, string tinhTrang, decimal phiPhat)
        {
            ...
            using (var tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    using (var cmd = new SqlCommand(@"SELECT ct.MaDauSach, ct.NgayTraThucTe, pm.NgayMuon, pm.NgayHenTra
                    ...
                    if (canPhat && phiPhat <= 0)
                        return KetQuaXuLy.Loi("Trường hợp trả trễ/mất/hư hỏng phải nhập phí phạt cụ thể lớn hơn 0 (BR09, BR10).");

                    using (var up = new SqlCommand(@"UPDATE ChiTietPhieuMuon
                    ...
                    // Không mất và không hư thì mới cộng lại tồn kho
                    if (!mat && !huHong)
                    {
                        using (var stock = new SqlCommand("UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo+1 WHERE MaDauSach=@Ma", cn, tx))
                        {
                    ...
                    if (canPhat)
                    {
                        ...
                        using (var phat = new SqlCommand(@"INSERT INTO PhieuPhat(MaPhieuPhat, MaChiTiet, MaNhanVien, NgayPhat, LyDo, PhiPhat)
