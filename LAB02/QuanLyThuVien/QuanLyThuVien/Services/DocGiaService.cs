...
namespace QuanLyThuVien.Services
{
    public class DocGiaService
    {
        ...
        public DataTable LayComboDocGia() =>
            Db.Query("SELECT MaDocGia, (Ho + ' ' + Ten) AS HoTen FROM DocGia ORDER BY Ho, Ten");
        ...
        public KetQuaXuLy Luu(DocGia dg, bool capNhat)
        {
            ...
            try
            {
                ...
                return n > 0
                    ...
                    : KetQuaXuLy.Loi("Không có dữ liệu thay đổi.");
            }
        ...
        public KetQuaXuLy CapThe(string maDocGia, DateTime ngayCap, DateTime hanSuDung, bool daDongLePhi)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    using (var check = new SqlCommand(@"SELECT COUNT(*) FROM TheDocGia WHERE MaDocGia=@Ma AND TrangThai=1 AND HanSuDung>=@NgayCap", cn, tx))
                    {
                        check.Parameters.AddWithValue("@Ma", maDocGia);
                        check.Parameters.AddWithValue("@NgayCap", ngayCap.Date);
                        if (Convert.ToInt32(check.ExecuteScalar()) > 0)
                            return KetQuaXuLy.Loi("Độc giả đang có một thẻ còn giá trị sử dụng (Quy tắc BR02).");
                    }

                    using (var off = new SqlCommand("UPDATE TheDocGia SET TrangThai=0 WHERE MaDocGia=@Ma AND TrangThai=1", cn, tx))
                    {
                    ...
                    using (var ins = new SqlCommand(@"INSERT INTO TheDocGia(MaThe, MaDocGia, NgayCap, HanSuDung, DaDongLePhi, TrangThai)
        ...
        {
            using (var cn = Db.OpenConnection())
            using (var cmd = new SqlCommand(@"SELECT TOP 1 MaThe, NgayCap, HanSuDung FROM TheDocGia WHERE MaDocGia=@Ma AND TrangThai=1 ORDER BY HanSuDung DESC", cn))
            {
                cmd.Parameters.AddWithValue("@Ma", maDocGia);
                using (var rd = cmd.ExecuteReader())
                {
                    if (!rd.Read()) return KetQuaXuLy.Loi("Độc giả chưa có thẻ đang hoạt động để gia hạn.");
                    ...
                    using (var up = new SqlCommand("UPDATE TheDocGia SET HanSuDung=@Han, DaDongLePhi=@LePhi WHERE MaThe=@MaThe", cn))
                    {
