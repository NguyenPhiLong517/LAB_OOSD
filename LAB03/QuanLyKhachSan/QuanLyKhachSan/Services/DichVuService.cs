...
namespace QuanLyKhachSan.Services
{
    public class DichVuService
    {
        public DataTable LayPhieuDangO() =>
            Db.Query(@"SELECT d.SoPhieuDat, k.HoTen, c.SoPhong 
        ...
        public DataTable LayLichSu(string so) =>
            Db.Query(@"SELECT p.SoPhieuSDDV, p.SoPhong, p.NgaySuDung, d.TenDV, c.SoLuong, c.DonGia, c.ThanhTien 
                      ...
                      ORDER BY p.NgaySuDung DESC, d.TenDV", new SqlParameter("@s", so));
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    if (Convert.ToString(st.ExecuteScalar()) != "Đang ở")
                        return KetQuaXuLy.Fail("Chỉ ghi nhận dịch vụ cho phòng đang lưu trú.");
                    ...
                    tx.Commit();
                    return KetQuaXuLy.Ok("Đã ghi nhận dịch vụ thành công.");
                }
                catch (Exception ex)
                {
                    tx.Rollback();
