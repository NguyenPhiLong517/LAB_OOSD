...
namespace QuanLyKhachSan.Services
{
    public class PhongTienNghiService
    {
        ...
        public DataTable LayTienNghi() => Db.Query("SELECT t.*, l.TenLoaiTN FROM TienNghi t JOIN LoaiTienNghi l ON t.MaLoaiTN=l.MaLoaiTN ORDER BY t.MaTienNghi");
        public DataTable LayLapDat() => Db.Query(@"SELECT p.*, l.TenLoaiTN 
                                                  FROM PhieuLapDat p 
        ...
        public KetQuaXuLy LapDat(string soPhieu, string maTN, string soPhong, DateTime ngay, string tinhTrang, string maNV, string ghiChu)
        {
            ...
            catch (SqlException ex)
            {
                if (ex.Number == 2627 || ex.Number == 2601)
                    return KetQuaXuLy.Fail("Thiết bị này đã được lắp cho một phòng khác trong ngày đã chọn (Quy tắc BR04).");
