...
namespace QuanLyKhachSan.Services
{
    public class DatPhongService
    {
        ...
        public DataTable LayChiTiet(string so) => Db.Query(@"SELECT c.*, p.SoNguoiToiDa, p.DonGiaNgay 
        ...
        public KetQuaXuLy ThemKhach(string ma, string ten, string cmnd, string qt, string sdt)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO KhachHang VALUES(@m, @t, @c, @q, @s)",
                    ...
                    new SqlParameter("@c", cmnd), new SqlParameter("@q", qt), new SqlParameter("@s", (object)sdt ?? DBNull.Value));
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    foreach (var x in ds)
                    {
                        ...
                        var maxCap = Convert.ToInt32(q.ExecuteScalar());
                        if (x.SoNguoi <= 0 || x.SoNguoi > maxCap)
                            return KetQuaXuLy.Fail($"Số người của phòng {x.SoPhong} vượt sức chứa tối đa ({maxCap}).");
        ...
        public KetQuaXuLy ThemNguoiLuuTru(string so, string phong, string ten, string cmnd, string qt)
        {
            ...
            try
            {
                ...
                if (dem >= max)
                    return KetQuaXuLy.Fail("Đã đủ số người đăng ký lưu trú cho phòng này.");
                ...
                Db.Execute("INSERT INTO NguoiLuuTru(SoPhieuDat, SoPhong, HoTen, SoCMND, QuocTich) VALUES(@s, @p, @t, @c, @q)",
                    ...
                    new SqlParameter("@t", ten), new SqlParameter("@c", cmnd), new SqlParameter("@q", qt));
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    if (c.ExecuteNonQuery() == 0)
                        return KetQuaXuLy.Fail("Phiếu không ở trạng thái 'Đã đặt' để nhận phòng.");
                    ...
                    tx.Commit();
                    return KetQuaXuLy.Ok("Đã hoàn tất nhận phòng.");
                }
                catch (Exception ex)
                {
                    tx.Rollback();
        ...
        public KetQuaXuLy DanhDauNoShow(string so)
        {
            try
            {
                ...
                return KetQuaXuLy.Ok("Đã đánh dấu No-show và giải phóng phòng.");
            }
            catch (Exception ex) { return KetQuaXuLy.Fail(ex.Message); }
        }
    }
