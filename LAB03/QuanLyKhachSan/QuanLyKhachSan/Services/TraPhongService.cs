...
namespace QuanLyKhachSan.Services
{
    public class TraPhongService
    {
        public DataTable LayPhieuDangO() =>
            Db.Query(@"SELECT d.SoPhieuDat, k.HoTen, d.NgayNhanThucTe, d.NgayTraDuKien 
        ...
        public DataTable LayPhongTheoPhieu(string so) =>
            Db.Query(@"SELECT c.SoPhong, p.DonGiaNgay 
        ...
        public KetQuaXuLy LapPhieuDenBu(string soDB, string soDat, string phong, DateTime ngay, string maNV, List<DenBuItem> ds)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    tx.Commit();
                    return KetQuaXuLy.Ok("Đã lập phiếu đền bù thành công.");
                }
                catch (Exception ex)
                {
                    tx.Rollback();
        ...
        public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay)
        {
            ...
            try
            {
                decimal donGiaPhong = Convert.ToDecimal(Db.Scalar(@"SELECT ISNULL(SUM(p.DonGiaNgay),0) 
                                                                   FROM ChiTietDatPhong c 
                                                                   JOIN Phong p ON c.SoPhong=p.SoPhong 
                                                                   WHERE c.SoPhieuDat=@s", new SqlParameter("@s", soDat)));
                decimal phong = donGiaPhong * soNgay;
        ...
        public KetQuaXuLy ThanhToan(string maTT, string soHD, DateTime ngay, string hinhThuc, decimal tien)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    if (da + tien > tong)
                        return KetQuaXuLy.Fail($"Số tiền vượt quá số cần thanh toán (Còn lại: {tong - da:N0} VNĐ).");
                    ...
                    tx.Commit();
                    return KetQuaXuLy.Ok($"Đã ghi nhận thanh toán ({hinhThuc}).");
                }
                catch (Exception ex)
                {
                    tx.Rollback();
        ...
        public KetQuaXuLy TraPhong(string soDat, DateTime ngayTra)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    var q = new SqlCommand("SELECT SoHoaDon, TrangThai FROM HoaDon WHERE SoPhieuDat=@s", cn, tx);
                    q.Parameters.AddWithValue("@s", soDat);
                    string tt = null;
                    using (var rd = q.ExecuteReader())
                    {
                        if (!rd.Read()) return KetQuaXuLy.Fail("Chưa lập hóa đơn cho phiếu đặt.");
                        tt = Convert.ToString(rd["TrangThai"]);
                    }

                    if (tt != "Đã thanh toán")
                        return KetQuaXuLy.Fail("Hóa đơn chưa thanh toán đủ, chưa thể trả phòng.");
